# 更新日志

所有显著变更都会记录在此文件中。

## v1.0.45 - 2026-08-28

### ✨ 新功能
- (无)

### 🐛 问题修复
- (无)

### 📦 其他变更
- [0;32m[INFO][0m 获取提交记录范围: v1.0.44..HEAD
- Fix settings not appearing + cleanup functions not working

## v1.0.44 - 2026-08-28

### ✨ 新功能
- (无)

### 🐛 问题修复
- (无)

### 📦 其他变更
- [0;32m[INFO][0m 获取提交记录范围: v1.0.43..HEAD
- Add Preference Bundle for iOS Settings app integration

## v1.0.43 - 2026-08-28

### ✨ 新功能
- (无)

### 🐛 问题修复
- (无)

### 📦 其他变更
- [0;32m[INFO][0m 获取提交记录范围: v1.0.42..HEAD
- Fix all cleanup functions: Security framework, rootless compat, bundleID passing

## v1.0.42 - 2026-08-28

### ✨ 新功能
- (无)

### 🐛 问题修复
- 修复 GhostKitOverlayWindow 重复符号
- 修复两个编译错误
- RootHelper 全面试证修复，确保各功能在 rootless 越狱下可用
- RootHelper setuid(0) 失败后继续执行而非退出
- 修复 .deb keyWindow 废弃错误 + 版本号不递增
- RootHelperManager 改用 SpawnBridge C 桥接调用 RootHelper
- 修复点击应用无反应 + 实现清理功能真实逻辑
- 修复 GhostKitSettingsViewController.m 指针类型和格式化错误
- 修复第二轮编译错误
- 修复编译错误
- 修复 OverlayController.m 编译错误 v4
- 修复 OverlayController.m 编译错误 v3
- 修复编译错误 v2
- 修复编译错误
- 调整 TARGET 版本以修复 CI 编译问题
- 移除降级策略，改为直接输出错误日志
- 修复 CI 编译失败 - 保持标准 THEOS 兼容性
- 双路径架构修复 - 支持 TrollStore(rootless) 和越狱环境
- 在 Tweak.x 中添加 DarwinNotifyManager 导入
- 移除重复的 DarwinCommandUnknown 枚举值
- 修复 DarwinNotifyManager 编译错误
- 修复OverlayController.m编译错误和逻辑bug
- 修复所有清理功能无效的问题
- 修复 DarwinNotifyManager.m 编译错误
- 修复 DarwinNotifyManager 枚举类型 nullable 编译错误
- 添加 DarwinNotifyManager.m 到 Makefile
- 从源码编译 RootHelper 为 64-bit arm64e
- 简化 RootHelper 构建流程，修复 CI 编译失败
- 修复 RootHelper 32-bit 架构问题，添加 3指长按悬浮面板，修复 iOS 16+ Apple ID 检测

### 📦 其他变更
- [0;32m[INFO][0m 获取提交记录范围: v1.0.16..HEAD
- chore(release): v1.0.41 [skip ci]
- chore(release): v1.0.40 [skip ci]
- chore(release): v1.0.39 [skip ci]
- chore(release): v1.0.38 [skip ci]
- chore(release): v1.0.37 [skip ci]
- chore(release): v1.0.36 [skip ci]
- chore(release): v1.0.35 [skip ci]
- chore(release): v1.0.34 [skip ci]
- chore(release): v1.0.33 [skip ci]
- chore(release): v1.0.32 [skip ci]
- chore(release): v1.0.31 [skip ci]
- chore(release): v1.0.30 [skip ci]
- chore(release): v1.0.29 [skip ci]
- chore(release): v1.0.28 [skip ci]
- chore(release): v1.0.27 [skip ci]
- chore(release): v1.0.26 [skip ci]
- chore(release): v1.0.25 [skip ci]
- chore(release): v1.0.24 [skip ci]
- chore(release): v1.0.23 [skip ci]
- chore(release): v1.0.22 [skip ci]
- chore(release): v1.0.21 [skip ci]
- chore(release): v1.0.20 [skip ci]
- chore(release): v1.0.19 [skip ci]
- chore(release): v1.0.18 [skip ci]
- chore(release): v1.0.17 [skip ci]

## v1.0.41 - 2026-08-28

### ✨ 新功能
- (无)

### 🐛 问题修复
- 修复两个编译错误
- RootHelper 全面试证修复，确保各功能在 rootless 越狱下可用
- RootHelper setuid(0) 失败后继续执行而非退出
- 修复 .deb keyWindow 废弃错误 + 版本号不递增
- RootHelperManager 改用 SpawnBridge C 桥接调用 RootHelper
- 修复点击应用无反应 + 实现清理功能真实逻辑
- 修复 GhostKitSettingsViewController.m 指针类型和格式化错误
- 修复第二轮编译错误
- 修复编译错误
- 修复 OverlayController.m 编译错误 v4
- 修复 OverlayController.m 编译错误 v3
- 修复编译错误 v2
- 修复编译错误
- 调整 TARGET 版本以修复 CI 编译问题
- 移除降级策略，改为直接输出错误日志
- 修复 CI 编译失败 - 保持标准 THEOS 兼容性
- 双路径架构修复 - 支持 TrollStore(rootless) 和越狱环境
- 在 Tweak.x 中添加 DarwinNotifyManager 导入
- 移除重复的 DarwinCommandUnknown 枚举值
- 修复 DarwinNotifyManager 编译错误
- 修复OverlayController.m编译错误和逻辑bug
- 修复所有清理功能无效的问题
- 修复 DarwinNotifyManager.m 编译错误
- 修复 DarwinNotifyManager 枚举类型 nullable 编译错误
- 添加 DarwinNotifyManager.m 到 Makefile
- 从源码编译 RootHelper 为 64-bit arm64e
- 简化 RootHelper 构建流程，修复 CI 编译失败
- 修复 RootHelper 32-bit 架构问题，添加 3指长按悬浮面板，修复 iOS 16+ Apple ID 检测

### 📦 其他变更
- [0;32m[INFO][0m 获取提交记录范围: v1.0.16..HEAD
- chore(release): v1.0.40 [skip ci]
- chore(release): v1.0.39 [skip ci]
- chore(release): v1.0.38 [skip ci]
- chore(release): v1.0.37 [skip ci]
- chore(release): v1.0.36 [skip ci]
- chore(release): v1.0.35 [skip ci]
- chore(release): v1.0.34 [skip ci]
- chore(release): v1.0.33 [skip ci]
- chore(release): v1.0.32 [skip ci]
- chore(release): v1.0.31 [skip ci]
- chore(release): v1.0.30 [skip ci]
- chore(release): v1.0.29 [skip ci]
- chore(release): v1.0.28 [skip ci]
- chore(release): v1.0.27 [skip ci]
- chore(release): v1.0.26 [skip ci]
- chore(release): v1.0.25 [skip ci]
- chore(release): v1.0.24 [skip ci]
- chore(release): v1.0.23 [skip ci]
- chore(release): v1.0.22 [skip ci]
- chore(release): v1.0.21 [skip ci]
- chore(release): v1.0.20 [skip ci]
- chore(release): v1.0.19 [skip ci]
- chore(release): v1.0.18 [skip ci]
- chore(release): v1.0.17 [skip ci]

## v1.0.40 - 2026-08-28

### ✨ 新功能
- (无)

### 🐛 问题修复
- RootHelper 全面试证修复，确保各功能在 rootless 越狱下可用
- RootHelper setuid(0) 失败后继续执行而非退出
- 修复 .deb keyWindow 废弃错误 + 版本号不递增
- RootHelperManager 改用 SpawnBridge C 桥接调用 RootHelper
- 修复点击应用无反应 + 实现清理功能真实逻辑
- 修复 GhostKitSettingsViewController.m 指针类型和格式化错误
- 修复第二轮编译错误
- 修复编译错误
- 修复 OverlayController.m 编译错误 v4
- 修复 OverlayController.m 编译错误 v3
- 修复编译错误 v2
- 修复编译错误
- 调整 TARGET 版本以修复 CI 编译问题
- 移除降级策略，改为直接输出错误日志
- 修复 CI 编译失败 - 保持标准 THEOS 兼容性
- 双路径架构修复 - 支持 TrollStore(rootless) 和越狱环境
- 在 Tweak.x 中添加 DarwinNotifyManager 导入
- 移除重复的 DarwinCommandUnknown 枚举值
- 修复 DarwinNotifyManager 编译错误
- 修复OverlayController.m编译错误和逻辑bug
- 修复所有清理功能无效的问题
- 修复 DarwinNotifyManager.m 编译错误
- 修复 DarwinNotifyManager 枚举类型 nullable 编译错误
- 添加 DarwinNotifyManager.m 到 Makefile
- 从源码编译 RootHelper 为 64-bit arm64e
- 简化 RootHelper 构建流程，修复 CI 编译失败
- 修复 RootHelper 32-bit 架构问题，添加 3指长按悬浮面板，修复 iOS 16+ Apple ID 检测

### 📦 其他变更
- [0;32m[INFO][0m 获取提交记录范围: v1.0.16..HEAD
- chore(release): v1.0.39 [skip ci]
- chore(release): v1.0.38 [skip ci]
- chore(release): v1.0.37 [skip ci]
- chore(release): v1.0.36 [skip ci]
- chore(release): v1.0.35 [skip ci]
- chore(release): v1.0.34 [skip ci]
- chore(release): v1.0.33 [skip ci]
- chore(release): v1.0.32 [skip ci]
- chore(release): v1.0.31 [skip ci]
- chore(release): v1.0.30 [skip ci]
- chore(release): v1.0.29 [skip ci]
- chore(release): v1.0.28 [skip ci]
- chore(release): v1.0.27 [skip ci]
- chore(release): v1.0.26 [skip ci]
- chore(release): v1.0.25 [skip ci]
- chore(release): v1.0.24 [skip ci]
- chore(release): v1.0.23 [skip ci]
- chore(release): v1.0.22 [skip ci]
- chore(release): v1.0.21 [skip ci]
- chore(release): v1.0.20 [skip ci]
- chore(release): v1.0.19 [skip ci]
- chore(release): v1.0.18 [skip ci]
- chore(release): v1.0.17 [skip ci]

## v1.0.39 - 2026-08-28

### ✨ 新功能
- (无)

### 🐛 问题修复
- RootHelper setuid(0) 失败后继续执行而非退出
- 修复 .deb keyWindow 废弃错误 + 版本号不递增
- RootHelperManager 改用 SpawnBridge C 桥接调用 RootHelper
- 修复点击应用无反应 + 实现清理功能真实逻辑
- 修复 GhostKitSettingsViewController.m 指针类型和格式化错误
- 修复第二轮编译错误
- 修复编译错误
- 修复 OverlayController.m 编译错误 v4
- 修复 OverlayController.m 编译错误 v3
- 修复编译错误 v2
- 修复编译错误
- 调整 TARGET 版本以修复 CI 编译问题
- 移除降级策略，改为直接输出错误日志
- 修复 CI 编译失败 - 保持标准 THEOS 兼容性
- 双路径架构修复 - 支持 TrollStore(rootless) 和越狱环境
- 在 Tweak.x 中添加 DarwinNotifyManager 导入
- 移除重复的 DarwinCommandUnknown 枚举值
- 修复 DarwinNotifyManager 编译错误
- 修复OverlayController.m编译错误和逻辑bug
- 修复所有清理功能无效的问题
- 修复 DarwinNotifyManager.m 编译错误
- 修复 DarwinNotifyManager 枚举类型 nullable 编译错误
- 添加 DarwinNotifyManager.m 到 Makefile
- 从源码编译 RootHelper 为 64-bit arm64e
- 简化 RootHelper 构建流程，修复 CI 编译失败
- 修复 RootHelper 32-bit 架构问题，添加 3指长按悬浮面板，修复 iOS 16+ Apple ID 检测

### 📦 其他变更
- [0;32m[INFO][0m 获取提交记录范围: v1.0.16..HEAD
- chore(release): v1.0.38 [skip ci]
- chore(release): v1.0.37 [skip ci]
- chore(release): v1.0.36 [skip ci]
- chore(release): v1.0.35 [skip ci]
- chore(release): v1.0.34 [skip ci]
- chore(release): v1.0.33 [skip ci]
- chore(release): v1.0.32 [skip ci]
- chore(release): v1.0.31 [skip ci]
- chore(release): v1.0.30 [skip ci]
- chore(release): v1.0.29 [skip ci]
- chore(release): v1.0.28 [skip ci]
- chore(release): v1.0.27 [skip ci]
- chore(release): v1.0.26 [skip ci]
- chore(release): v1.0.25 [skip ci]
- chore(release): v1.0.24 [skip ci]
- chore(release): v1.0.23 [skip ci]
- chore(release): v1.0.22 [skip ci]
- chore(release): v1.0.21 [skip ci]
- chore(release): v1.0.20 [skip ci]
- chore(release): v1.0.19 [skip ci]
- chore(release): v1.0.18 [skip ci]
- chore(release): v1.0.17 [skip ci]

## v1.0.38 - 2026-08-28

### ✨ 新功能
- (无)

### 🐛 问题修复
- 修复 .deb keyWindow 废弃错误 + 版本号不递增
- RootHelperManager 改用 SpawnBridge C 桥接调用 RootHelper
- 修复点击应用无反应 + 实现清理功能真实逻辑
- 修复 GhostKitSettingsViewController.m 指针类型和格式化错误
- 修复第二轮编译错误
- 修复编译错误
- 修复 OverlayController.m 编译错误 v4
- 修复 OverlayController.m 编译错误 v3
- 修复编译错误 v2
- 修复编译错误
- 调整 TARGET 版本以修复 CI 编译问题
- 移除降级策略，改为直接输出错误日志
- 修复 CI 编译失败 - 保持标准 THEOS 兼容性
- 双路径架构修复 - 支持 TrollStore(rootless) 和越狱环境
- 在 Tweak.x 中添加 DarwinNotifyManager 导入
- 移除重复的 DarwinCommandUnknown 枚举值
- 修复 DarwinNotifyManager 编译错误
- 修复OverlayController.m编译错误和逻辑bug
- 修复所有清理功能无效的问题
- 修复 DarwinNotifyManager.m 编译错误
- 修复 DarwinNotifyManager 枚举类型 nullable 编译错误
- 添加 DarwinNotifyManager.m 到 Makefile
- 从源码编译 RootHelper 为 64-bit arm64e
- 简化 RootHelper 构建流程，修复 CI 编译失败
- 修复 RootHelper 32-bit 架构问题，添加 3指长按悬浮面板，修复 iOS 16+ Apple ID 检测

### 📦 其他变更
- [0;32m[INFO][0m 获取提交记录范围: v1.0.16..HEAD
- chore(release): v1.0.37 [skip ci]
- chore(release): v1.0.36 [skip ci]
- chore(release): v1.0.35 [skip ci]
- chore(release): v1.0.34 [skip ci]
- chore(release): v1.0.33 [skip ci]
- chore(release): v1.0.32 [skip ci]
- chore(release): v1.0.31 [skip ci]
- chore(release): v1.0.30 [skip ci]
- chore(release): v1.0.29 [skip ci]
- chore(release): v1.0.28 [skip ci]
- chore(release): v1.0.27 [skip ci]
- chore(release): v1.0.26 [skip ci]
- chore(release): v1.0.25 [skip ci]
- chore(release): v1.0.24 [skip ci]
- chore(release): v1.0.23 [skip ci]
- chore(release): v1.0.22 [skip ci]
- chore(release): v1.0.21 [skip ci]
- chore(release): v1.0.20 [skip ci]
- chore(release): v1.0.19 [skip ci]
- chore(release): v1.0.18 [skip ci]
- chore(release): v1.0.17 [skip ci]

## v1.0.37 - 2026-08-28

### ✨ 新功能
- (无)

### 🐛 问题修复
- RootHelperManager 改用 SpawnBridge C 桥接调用 RootHelper
- 修复点击应用无反应 + 实现清理功能真实逻辑
- 修复 GhostKitSettingsViewController.m 指针类型和格式化错误
- 修复第二轮编译错误
- 修复编译错误
- 修复 OverlayController.m 编译错误 v4
- 修复 OverlayController.m 编译错误 v3
- 修复编译错误 v2
- 修复编译错误
- 调整 TARGET 版本以修复 CI 编译问题
- 移除降级策略，改为直接输出错误日志
- 修复 CI 编译失败 - 保持标准 THEOS 兼容性
- 双路径架构修复 - 支持 TrollStore(rootless) 和越狱环境
- 在 Tweak.x 中添加 DarwinNotifyManager 导入
- 移除重复的 DarwinCommandUnknown 枚举值
- 修复 DarwinNotifyManager 编译错误
- 修复OverlayController.m编译错误和逻辑bug
- 修复所有清理功能无效的问题
- 修复 DarwinNotifyManager.m 编译错误
- 修复 DarwinNotifyManager 枚举类型 nullable 编译错误
- 添加 DarwinNotifyManager.m 到 Makefile
- 从源码编译 RootHelper 为 64-bit arm64e
- 简化 RootHelper 构建流程，修复 CI 编译失败
- 修复 RootHelper 32-bit 架构问题，添加 3指长按悬浮面板，修复 iOS 16+ Apple ID 检测

### 📦 其他变更
- [0;32m[INFO][0m 获取提交记录范围: v1.0.16..HEAD
- chore(release): v1.0.36 [skip ci]
- chore(release): v1.0.35 [skip ci]
- chore(release): v1.0.34 [skip ci]
- chore(release): v1.0.33 [skip ci]
- chore(release): v1.0.32 [skip ci]
- chore(release): v1.0.31 [skip ci]
- chore(release): v1.0.30 [skip ci]
- chore(release): v1.0.29 [skip ci]
- chore(release): v1.0.28 [skip ci]
- chore(release): v1.0.27 [skip ci]
- chore(release): v1.0.26 [skip ci]
- chore(release): v1.0.25 [skip ci]
- chore(release): v1.0.24 [skip ci]
- chore(release): v1.0.23 [skip ci]
- chore(release): v1.0.22 [skip ci]
- chore(release): v1.0.21 [skip ci]
- chore(release): v1.0.20 [skip ci]
- chore(release): v1.0.19 [skip ci]
- chore(release): v1.0.18 [skip ci]
- chore(release): v1.0.17 [skip ci]

## v1.0.36 - 2026-08-28

### ✨ 新功能
- (无)

### 🐛 问题修复
- 修复点击应用无反应 + 实现清理功能真实逻辑
- 修复 GhostKitSettingsViewController.m 指针类型和格式化错误
- 修复第二轮编译错误
- 修复编译错误
- 修复 OverlayController.m 编译错误 v4
- 修复 OverlayController.m 编译错误 v3
- 修复编译错误 v2
- 修复编译错误
- 调整 TARGET 版本以修复 CI 编译问题
- 移除降级策略，改为直接输出错误日志
- 修复 CI 编译失败 - 保持标准 THEOS 兼容性
- 双路径架构修复 - 支持 TrollStore(rootless) 和越狱环境
- 在 Tweak.x 中添加 DarwinNotifyManager 导入
- 移除重复的 DarwinCommandUnknown 枚举值
- 修复 DarwinNotifyManager 编译错误
- 修复OverlayController.m编译错误和逻辑bug
- 修复所有清理功能无效的问题
- 修复 DarwinNotifyManager.m 编译错误
- 修复 DarwinNotifyManager 枚举类型 nullable 编译错误
- 添加 DarwinNotifyManager.m 到 Makefile
- 从源码编译 RootHelper 为 64-bit arm64e
- 简化 RootHelper 构建流程，修复 CI 编译失败
- 修复 RootHelper 32-bit 架构问题，添加 3指长按悬浮面板，修复 iOS 16+ Apple ID 检测

### 📦 其他变更
- [0;32m[INFO][0m 获取提交记录范围: v1.0.16..HEAD
- chore(release): v1.0.35 [skip ci]
- chore(release): v1.0.34 [skip ci]
- chore(release): v1.0.33 [skip ci]
- chore(release): v1.0.32 [skip ci]
- chore(release): v1.0.31 [skip ci]
- chore(release): v1.0.30 [skip ci]
- chore(release): v1.0.29 [skip ci]
- chore(release): v1.0.28 [skip ci]
- chore(release): v1.0.27 [skip ci]
- chore(release): v1.0.26 [skip ci]
- chore(release): v1.0.25 [skip ci]
- chore(release): v1.0.24 [skip ci]
- chore(release): v1.0.23 [skip ci]
- chore(release): v1.0.22 [skip ci]
- chore(release): v1.0.21 [skip ci]
- chore(release): v1.0.20 [skip ci]
- chore(release): v1.0.19 [skip ci]
- chore(release): v1.0.18 [skip ci]
- chore(release): v1.0.17 [skip ci]

## v1.0.35 - 2026-08-28

### ✨ 新功能
- (无)

### 🐛 问题修复
- 修复 GhostKitSettingsViewController.m 指针类型和格式化错误
- 修复第二轮编译错误
- 修复编译错误
- 修复 OverlayController.m 编译错误 v4
- 修复 OverlayController.m 编译错误 v3
- 修复编译错误 v2
- 修复编译错误
- 调整 TARGET 版本以修复 CI 编译问题
- 移除降级策略，改为直接输出错误日志
- 修复 CI 编译失败 - 保持标准 THEOS 兼容性
- 双路径架构修复 - 支持 TrollStore(rootless) 和越狱环境
- 在 Tweak.x 中添加 DarwinNotifyManager 导入
- 移除重复的 DarwinCommandUnknown 枚举值
- 修复 DarwinNotifyManager 编译错误
- 修复OverlayController.m编译错误和逻辑bug
- 修复所有清理功能无效的问题
- 修复 DarwinNotifyManager.m 编译错误
- 修复 DarwinNotifyManager 枚举类型 nullable 编译错误
- 添加 DarwinNotifyManager.m 到 Makefile
- 从源码编译 RootHelper 为 64-bit arm64e
- 简化 RootHelper 构建流程，修复 CI 编译失败
- 修复 RootHelper 32-bit 架构问题，添加 3指长按悬浮面板，修复 iOS 16+ Apple ID 检测

### 📦 其他变更
- [0;32m[INFO][0m 获取提交记录范围: v1.0.16..HEAD
- chore(release): v1.0.34 [skip ci]
- chore(release): v1.0.33 [skip ci]
- chore(release): v1.0.32 [skip ci]
- chore(release): v1.0.31 [skip ci]
- chore(release): v1.0.30 [skip ci]
- chore(release): v1.0.29 [skip ci]
- chore(release): v1.0.28 [skip ci]
- chore(release): v1.0.27 [skip ci]
- chore(release): v1.0.26 [skip ci]
- chore(release): v1.0.25 [skip ci]
- chore(release): v1.0.24 [skip ci]
- chore(release): v1.0.23 [skip ci]
- chore(release): v1.0.22 [skip ci]
- chore(release): v1.0.21 [skip ci]
- chore(release): v1.0.20 [skip ci]
- chore(release): v1.0.19 [skip ci]
- chore(release): v1.0.18 [skip ci]
- chore(release): v1.0.17 [skip ci]

## v1.0.34 - 2026-08-28

### ✨ 新功能
- (无)

### 🐛 问题修复
- 修复第二轮编译错误
- 修复编译错误
- 修复 OverlayController.m 编译错误 v4
- 修复 OverlayController.m 编译错误 v3
- 修复编译错误 v2
- 修复编译错误
- 调整 TARGET 版本以修复 CI 编译问题
- 移除降级策略，改为直接输出错误日志
- 修复 CI 编译失败 - 保持标准 THEOS 兼容性
- 双路径架构修复 - 支持 TrollStore(rootless) 和越狱环境
- 在 Tweak.x 中添加 DarwinNotifyManager 导入
- 移除重复的 DarwinCommandUnknown 枚举值
- 修复 DarwinNotifyManager 编译错误
- 修复OverlayController.m编译错误和逻辑bug
- 修复所有清理功能无效的问题
- 修复 DarwinNotifyManager.m 编译错误
- 修复 DarwinNotifyManager 枚举类型 nullable 编译错误
- 添加 DarwinNotifyManager.m 到 Makefile
- 从源码编译 RootHelper 为 64-bit arm64e
- 简化 RootHelper 构建流程，修复 CI 编译失败
- 修复 RootHelper 32-bit 架构问题，添加 3指长按悬浮面板，修复 iOS 16+ Apple ID 检测

### 📦 其他变更
- [0;32m[INFO][0m 获取提交记录范围: v1.0.16..HEAD
- chore(release): v1.0.33 [skip ci]
- chore(release): v1.0.32 [skip ci]
- chore(release): v1.0.31 [skip ci]
- chore(release): v1.0.30 [skip ci]
- chore(release): v1.0.29 [skip ci]
- chore(release): v1.0.28 [skip ci]
- chore(release): v1.0.27 [skip ci]
- chore(release): v1.0.26 [skip ci]
- chore(release): v1.0.25 [skip ci]
- chore(release): v1.0.24 [skip ci]
- chore(release): v1.0.23 [skip ci]
- chore(release): v1.0.22 [skip ci]
- chore(release): v1.0.21 [skip ci]
- chore(release): v1.0.20 [skip ci]
- chore(release): v1.0.19 [skip ci]
- chore(release): v1.0.18 [skip ci]
- chore(release): v1.0.17 [skip ci]

## v1.0.33 - 2026-08-28

### ✨ 新功能
- (无)

### 🐛 问题修复
- 修复编译错误
- 修复 OverlayController.m 编译错误 v4
- 修复 OverlayController.m 编译错误 v3
- 修复编译错误 v2
- 修复编译错误
- 调整 TARGET 版本以修复 CI 编译问题
- 移除降级策略，改为直接输出错误日志
- 修复 CI 编译失败 - 保持标准 THEOS 兼容性
- 双路径架构修复 - 支持 TrollStore(rootless) 和越狱环境
- 在 Tweak.x 中添加 DarwinNotifyManager 导入
- 移除重复的 DarwinCommandUnknown 枚举值
- 修复 DarwinNotifyManager 编译错误
- 修复OverlayController.m编译错误和逻辑bug
- 修复所有清理功能无效的问题
- 修复 DarwinNotifyManager.m 编译错误
- 修复 DarwinNotifyManager 枚举类型 nullable 编译错误
- 添加 DarwinNotifyManager.m 到 Makefile
- 从源码编译 RootHelper 为 64-bit arm64e
- 简化 RootHelper 构建流程，修复 CI 编译失败
- 修复 RootHelper 32-bit 架构问题，添加 3指长按悬浮面板，修复 iOS 16+ Apple ID 检测

### 📦 其他变更
- [0;32m[INFO][0m 获取提交记录范围: v1.0.16..HEAD
- chore(release): v1.0.32 [skip ci]
- chore(release): v1.0.31 [skip ci]
- chore(release): v1.0.30 [skip ci]
- chore(release): v1.0.29 [skip ci]
- chore(release): v1.0.28 [skip ci]
- chore(release): v1.0.27 [skip ci]
- chore(release): v1.0.26 [skip ci]
- chore(release): v1.0.25 [skip ci]
- chore(release): v1.0.24 [skip ci]
- chore(release): v1.0.23 [skip ci]
- chore(release): v1.0.22 [skip ci]
- chore(release): v1.0.21 [skip ci]
- chore(release): v1.0.20 [skip ci]
- chore(release): v1.0.19 [skip ci]
- chore(release): v1.0.18 [skip ci]
- chore(release): v1.0.17 [skip ci]

## v1.0.32 - 2026-08-24

### ✨ 新功能
- (无)

### 🐛 问题修复
- 修复 OverlayController.m 编译错误 v4
- 修复 OverlayController.m 编译错误 v3
- 修复编译错误 v2
- 修复编译错误
- 调整 TARGET 版本以修复 CI 编译问题
- 移除降级策略，改为直接输出错误日志
- 修复 CI 编译失败 - 保持标准 THEOS 兼容性
- 双路径架构修复 - 支持 TrollStore(rootless) 和越狱环境
- 在 Tweak.x 中添加 DarwinNotifyManager 导入
- 移除重复的 DarwinCommandUnknown 枚举值
- 修复 DarwinNotifyManager 编译错误
- 修复OverlayController.m编译错误和逻辑bug
- 修复所有清理功能无效的问题
- 修复 DarwinNotifyManager.m 编译错误
- 修复 DarwinNotifyManager 枚举类型 nullable 编译错误
- 添加 DarwinNotifyManager.m 到 Makefile
- 从源码编译 RootHelper 为 64-bit arm64e
- 简化 RootHelper 构建流程，修复 CI 编译失败
- 修复 RootHelper 32-bit 架构问题，添加 3指长按悬浮面板，修复 iOS 16+ Apple ID 检测

### 📦 其他变更
- [0;32m[INFO][0m 获取提交记录范围: v1.0.16..HEAD
- chore(release): v1.0.31 [skip ci]
- chore(release): v1.0.30 [skip ci]
- chore(release): v1.0.29 [skip ci]
- chore(release): v1.0.28 [skip ci]
- chore(release): v1.0.27 [skip ci]
- chore(release): v1.0.26 [skip ci]
- chore(release): v1.0.25 [skip ci]
- chore(release): v1.0.24 [skip ci]
- chore(release): v1.0.23 [skip ci]
- chore(release): v1.0.22 [skip ci]
- chore(release): v1.0.21 [skip ci]
- chore(release): v1.0.20 [skip ci]
- chore(release): v1.0.19 [skip ci]
- chore(release): v1.0.18 [skip ci]
- chore(release): v1.0.17 [skip ci]

## v1.0.31 - 2026-08-24

### ✨ 新功能
- (无)

### 🐛 问题修复
- 调整 TARGET 版本以修复 CI 编译问题
- 移除降级策略，改为直接输出错误日志
- 修复 CI 编译失败 - 保持标准 THEOS 兼容性
- 双路径架构修复 - 支持 TrollStore(rootless) 和越狱环境
- 在 Tweak.x 中添加 DarwinNotifyManager 导入
- 移除重复的 DarwinCommandUnknown 枚举值
- 修复 DarwinNotifyManager 编译错误
- 修复OverlayController.m编译错误和逻辑bug
- 修复所有清理功能无效的问题
- 修复 DarwinNotifyManager.m 编译错误
- 修复 DarwinNotifyManager 枚举类型 nullable 编译错误
- 添加 DarwinNotifyManager.m 到 Makefile
- 从源码编译 RootHelper 为 64-bit arm64e
- 简化 RootHelper 构建流程，修复 CI 编译失败
- 修复 RootHelper 32-bit 架构问题，添加 3指长按悬浮面板，修复 iOS 16+ Apple ID 检测

### 📦 其他变更
- [0;32m[INFO][0m 获取提交记录范围: v1.0.16..HEAD
- chore(release): v1.0.30 [skip ci]
- chore(release): v1.0.29 [skip ci]
- chore(release): v1.0.28 [skip ci]
- chore(release): v1.0.27 [skip ci]
- chore(release): v1.0.26 [skip ci]
- chore(release): v1.0.25 [skip ci]
- chore(release): v1.0.24 [skip ci]
- chore(release): v1.0.23 [skip ci]
- chore(release): v1.0.22 [skip ci]
- chore(release): v1.0.21 [skip ci]
- chore(release): v1.0.20 [skip ci]
- chore(release): v1.0.19 [skip ci]
- chore(release): v1.0.18 [skip ci]
- chore(release): v1.0.17 [skip ci]

## v1.0.30 - 2026-08-23

### ✨ 新功能
- (无)

### 🐛 问题修复
- 修复 CI 编译失败 - 保持标准 THEOS 兼容性
- 双路径架构修复 - 支持 TrollStore(rootless) 和越狱环境
- 在 Tweak.x 中添加 DarwinNotifyManager 导入
- 移除重复的 DarwinCommandUnknown 枚举值
- 修复 DarwinNotifyManager 编译错误
- 修复OverlayController.m编译错误和逻辑bug
- 修复所有清理功能无效的问题
- 修复 DarwinNotifyManager.m 编译错误
- 修复 DarwinNotifyManager 枚举类型 nullable 编译错误
- 添加 DarwinNotifyManager.m 到 Makefile
- 从源码编译 RootHelper 为 64-bit arm64e
- 简化 RootHelper 构建流程，修复 CI 编译失败
- 修复 RootHelper 32-bit 架构问题，添加 3指长按悬浮面板，修复 iOS 16+ Apple ID 检测

### 📦 其他变更
- [0;32m[INFO][0m 获取提交记录范围: v1.0.16..HEAD
- chore(release): v1.0.29 [skip ci]
- chore(release): v1.0.28 [skip ci]
- chore(release): v1.0.27 [skip ci]
- chore(release): v1.0.26 [skip ci]
- chore(release): v1.0.25 [skip ci]
- chore(release): v1.0.24 [skip ci]
- chore(release): v1.0.23 [skip ci]
- chore(release): v1.0.22 [skip ci]
- chore(release): v1.0.21 [skip ci]
- chore(release): v1.0.20 [skip ci]
- chore(release): v1.0.19 [skip ci]
- chore(release): v1.0.18 [skip ci]
- chore(release): v1.0.17 [skip ci]

## v1.0.29 - 2026-08-23

### ✨ 新功能
- (无)

### 🐛 问题修复
- 修复 CI 编译失败 - 保持标准 THEOS 兼容性
- 双路径架构修复 - 支持 TrollStore(rootless) 和越狱环境
- 在 Tweak.x 中添加 DarwinNotifyManager 导入
- 移除重复的 DarwinCommandUnknown 枚举值
- 修复 DarwinNotifyManager 编译错误
- 修复OverlayController.m编译错误和逻辑bug
- 修复所有清理功能无效的问题
- 修复 DarwinNotifyManager.m 编译错误
- 修复 DarwinNotifyManager 枚举类型 nullable 编译错误
- 添加 DarwinNotifyManager.m 到 Makefile
- 从源码编译 RootHelper 为 64-bit arm64e
- 简化 RootHelper 构建流程，修复 CI 编译失败
- 修复 RootHelper 32-bit 架构问题，添加 3指长按悬浮面板，修复 iOS 16+ Apple ID 检测

### 📦 其他变更
- [0;32m[INFO][0m 获取提交记录范围: v1.0.16..HEAD
- chore(release): v1.0.28 [skip ci]
- chore(release): v1.0.27 [skip ci]
- chore(release): v1.0.26 [skip ci]
- chore(release): v1.0.25 [skip ci]
- chore(release): v1.0.24 [skip ci]
- chore(release): v1.0.23 [skip ci]
- chore(release): v1.0.22 [skip ci]
- chore(release): v1.0.21 [skip ci]
- chore(release): v1.0.20 [skip ci]
- chore(release): v1.0.19 [skip ci]
- chore(release): v1.0.18 [skip ci]
- chore(release): v1.0.17 [skip ci]

## v1.0.28 - 2026-08-23

### ✨ 新功能
- (无)

### 🐛 问题修复
- 双路径架构修复 - 支持 TrollStore(rootless) 和越狱环境
- 在 Tweak.x 中添加 DarwinNotifyManager 导入
- 移除重复的 DarwinCommandUnknown 枚举值
- 修复 DarwinNotifyManager 编译错误
- 修复OverlayController.m编译错误和逻辑bug
- 修复所有清理功能无效的问题
- 修复 DarwinNotifyManager.m 编译错误
- 修复 DarwinNotifyManager 枚举类型 nullable 编译错误
- 添加 DarwinNotifyManager.m 到 Makefile
- 从源码编译 RootHelper 为 64-bit arm64e
- 简化 RootHelper 构建流程，修复 CI 编译失败
- 修复 RootHelper 32-bit 架构问题，添加 3指长按悬浮面板，修复 iOS 16+ Apple ID 检测

### 📦 其他变更
- [0;32m[INFO][0m 获取提交记录范围: v1.0.16..HEAD
- chore(release): v1.0.27 [skip ci]
- chore(release): v1.0.26 [skip ci]
- chore(release): v1.0.25 [skip ci]
- chore(release): v1.0.24 [skip ci]
- chore(release): v1.0.23 [skip ci]
- chore(release): v1.0.22 [skip ci]
- chore(release): v1.0.21 [skip ci]
- chore(release): v1.0.20 [skip ci]
- chore(release): v1.0.19 [skip ci]
- chore(release): v1.0.18 [skip ci]
- chore(release): v1.0.17 [skip ci]

## v1.0.27 - 2026-08-21

### ✨ 新功能
- (无)

### 🐛 问题修复
- 在 Tweak.x 中添加 DarwinNotifyManager 导入
- 移除重复的 DarwinCommandUnknown 枚举值
- 修复 DarwinNotifyManager 编译错误
- 修复OverlayController.m编译错误和逻辑bug
- 修复所有清理功能无效的问题
- 修复 DarwinNotifyManager.m 编译错误
- 修复 DarwinNotifyManager 枚举类型 nullable 编译错误
- 添加 DarwinNotifyManager.m 到 Makefile
- 从源码编译 RootHelper 为 64-bit arm64e
- 简化 RootHelper 构建流程，修复 CI 编译失败
- 修复 RootHelper 32-bit 架构问题，添加 3指长按悬浮面板，修复 iOS 16+ Apple ID 检测

### 📦 其他变更
- [0;32m[INFO][0m 获取提交记录范围: v1.0.16..HEAD
- chore(release): v1.0.26 [skip ci]
- chore(release): v1.0.25 [skip ci]
- chore(release): v1.0.24 [skip ci]
- chore(release): v1.0.23 [skip ci]
- chore(release): v1.0.22 [skip ci]
- chore(release): v1.0.21 [skip ci]
- chore(release): v1.0.20 [skip ci]
- chore(release): v1.0.19 [skip ci]
- chore(release): v1.0.18 [skip ci]
- chore(release): v1.0.17 [skip ci]

## v1.0.26 - 2026-08-21

### ✨ 新功能
- (无)

### 🐛 问题修复
- 移除重复的 DarwinCommandUnknown 枚举值
- 修复 DarwinNotifyManager 编译错误
- 修复OverlayController.m编译错误和逻辑bug
- 修复所有清理功能无效的问题
- 修复 DarwinNotifyManager.m 编译错误
- 修复 DarwinNotifyManager 枚举类型 nullable 编译错误
- 添加 DarwinNotifyManager.m 到 Makefile
- 从源码编译 RootHelper 为 64-bit arm64e
- 简化 RootHelper 构建流程，修复 CI 编译失败
- 修复 RootHelper 32-bit 架构问题，添加 3指长按悬浮面板，修复 iOS 16+ Apple ID 检测

### 📦 其他变更
- [0;32m[INFO][0m 获取提交记录范围: v1.0.16..HEAD
- chore(release): v1.0.25 [skip ci]
- chore(release): v1.0.24 [skip ci]
- chore(release): v1.0.23 [skip ci]
- chore(release): v1.0.22 [skip ci]
- chore(release): v1.0.21 [skip ci]
- chore(release): v1.0.20 [skip ci]
- chore(release): v1.0.19 [skip ci]
- chore(release): v1.0.18 [skip ci]
- chore(release): v1.0.17 [skip ci]

## v1.0.25 - 2026-08-21

### ✨ 新功能
- (无)

### 🐛 问题修复
- 修复 DarwinNotifyManager 编译错误
- 修复OverlayController.m编译错误和逻辑bug
- 修复所有清理功能无效的问题
- 修复 DarwinNotifyManager.m 编译错误
- 修复 DarwinNotifyManager 枚举类型 nullable 编译错误
- 添加 DarwinNotifyManager.m 到 Makefile
- 从源码编译 RootHelper 为 64-bit arm64e
- 简化 RootHelper 构建流程，修复 CI 编译失败
- 修复 RootHelper 32-bit 架构问题，添加 3指长按悬浮面板，修复 iOS 16+ Apple ID 检测

### 📦 其他变更
- [0;32m[INFO][0m 获取提交记录范围: v1.0.16..HEAD
- chore(release): v1.0.24 [skip ci]
- chore(release): v1.0.23 [skip ci]
- chore(release): v1.0.22 [skip ci]
- chore(release): v1.0.21 [skip ci]
- chore(release): v1.0.20 [skip ci]
- chore(release): v1.0.19 [skip ci]
- chore(release): v1.0.18 [skip ci]
- chore(release): v1.0.17 [skip ci]

## v1.0.24 - 2026-08-21

### ✨ 新功能
- (无)

### 🐛 问题修复
- 修复OverlayController.m编译错误和逻辑bug
- 修复所有清理功能无效的问题
- 修复 DarwinNotifyManager.m 编译错误
- 修复 DarwinNotifyManager 枚举类型 nullable 编译错误
- 添加 DarwinNotifyManager.m 到 Makefile
- 从源码编译 RootHelper 为 64-bit arm64e
- 简化 RootHelper 构建流程，修复 CI 编译失败
- 修复 RootHelper 32-bit 架构问题，添加 3指长按悬浮面板，修复 iOS 16+ Apple ID 检测

### 📦 其他变更
- [0;32m[INFO][0m 获取提交记录范围: v1.0.16..HEAD
- chore(release): v1.0.23 [skip ci]
- chore(release): v1.0.22 [skip ci]
- chore(release): v1.0.21 [skip ci]
- chore(release): v1.0.20 [skip ci]
- chore(release): v1.0.19 [skip ci]
- chore(release): v1.0.18 [skip ci]
- chore(release): v1.0.17 [skip ci]

## v1.0.23 - 2026-08-20

### ✨ 新功能
- (无)

### 🐛 问题修复
- 修复所有清理功能无效的问题
- 修复 DarwinNotifyManager.m 编译错误
- 修复 DarwinNotifyManager 枚举类型 nullable 编译错误
- 添加 DarwinNotifyManager.m 到 Makefile
- 从源码编译 RootHelper 为 64-bit arm64e
- 简化 RootHelper 构建流程，修复 CI 编译失败
- 修复 RootHelper 32-bit 架构问题，添加 3指长按悬浮面板，修复 iOS 16+ Apple ID 检测

### 📦 其他变更
- [0;32m[INFO][0m 获取提交记录范围: v1.0.16..HEAD
- chore(release): v1.0.22 [skip ci]
- chore(release): v1.0.21 [skip ci]
- chore(release): v1.0.20 [skip ci]
- chore(release): v1.0.19 [skip ci]
- chore(release): v1.0.18 [skip ci]
- chore(release): v1.0.17 [skip ci]

## v1.0.22 - 2026-08-20

### ✨ 新功能
- (无)

### 🐛 问题修复
- 修复 DarwinNotifyManager.m 编译错误
- 修复 DarwinNotifyManager 枚举类型 nullable 编译错误
- 添加 DarwinNotifyManager.m 到 Makefile
- 从源码编译 RootHelper 为 64-bit arm64e
- 简化 RootHelper 构建流程，修复 CI 编译失败
- 修复 RootHelper 32-bit 架构问题，添加 3指长按悬浮面板，修复 iOS 16+ Apple ID 检测

### 📦 其他变更
- [0;32m[INFO][0m 获取提交记录范围: v1.0.16..HEAD
- chore(release): v1.0.21 [skip ci]
- chore(release): v1.0.20 [skip ci]
- chore(release): v1.0.19 [skip ci]
- chore(release): v1.0.18 [skip ci]
- chore(release): v1.0.17 [skip ci]

## v1.0.21 - 2026-08-20

### ✨ 新功能
- (无)

### 🐛 问题修复
- 修复 DarwinNotifyManager 枚举类型 nullable 编译错误
- 添加 DarwinNotifyManager.m 到 Makefile
- 从源码编译 RootHelper 为 64-bit arm64e
- 简化 RootHelper 构建流程，修复 CI 编译失败
- 修复 RootHelper 32-bit 架构问题，添加 3指长按悬浮面板，修复 iOS 16+ Apple ID 检测

### 📦 其他变更
- [0;32m[INFO][0m 获取提交记录范围: v1.0.16..HEAD
- chore(release): v1.0.20 [skip ci]
- chore(release): v1.0.19 [skip ci]
- chore(release): v1.0.18 [skip ci]
- chore(release): v1.0.17 [skip ci]

## v1.0.20 - 2026-08-20

### ✨ 新功能
- (无)

### 🐛 问题修复
- 添加 DarwinNotifyManager.m 到 Makefile
- 从源码编译 RootHelper 为 64-bit arm64e
- 简化 RootHelper 构建流程，修复 CI 编译失败
- 修复 RootHelper 32-bit 架构问题，添加 3指长按悬浮面板，修复 iOS 16+ Apple ID 检测

### 📦 其他变更
- [0;32m[INFO][0m 获取提交记录范围: v1.0.16..HEAD
- chore(release): v1.0.19 [skip ci]
- chore(release): v1.0.18 [skip ci]
- chore(release): v1.0.17 [skip ci]

## v1.0.19 - 2026-08-20

### ✨ 新功能
- (无)

### 🐛 问题修复
- 从源码编译 RootHelper 为 64-bit arm64e
- 简化 RootHelper 构建流程，修复 CI 编译失败
- 修复 RootHelper 32-bit 架构问题，添加 3指长按悬浮面板，修复 iOS 16+ Apple ID 检测

### 📦 其他变更
- [0;32m[INFO][0m 获取提交记录范围: v1.0.16..HEAD
- chore(release): v1.0.18 [skip ci]
- chore(release): v1.0.17 [skip ci]

## v1.0.18 - 2026-08-20

### ✨ 新功能
- (无)

### 🐛 问题修复
- 简化 RootHelper 构建流程，修复 CI 编译失败
- 修复 RootHelper 32-bit 架构问题，添加 3指长按悬浮面板，修复 iOS 16+ Apple ID 检测

### 📦 其他变更
- [0;32m[INFO][0m 获取提交记录范围: v1.0.16..HEAD
- chore(release): v1.0.17 [skip ci]

## v1.0.17 - 2026-08-20

### ✨ 新功能
- (无)

### 🐛 问题修复
- 修复 RootHelper 32-bit 架构问题，添加 3指长按悬浮面板，修复 iOS 16+ Apple ID 检测

### 📦 其他变更
- [0;32m[INFO][0m 获取提交记录范围: v1.0.16..HEAD

## v1.0.16 - 2026-08-19

### ✨ 新功能
- (无)

### 🐛 问题修复
- (无)

### 📦 其他变更
- [0;32m[INFO][0m 获取提交记录范围: v1.0.14..HEAD
- Fix Tweak.x build error: const void* -> void* for CFNotificationCallback
- chore(release): v1.0.15 [skip ci]
- Fix all 11 identified issues

## v1.0.15 - 2026-08-19

### ✨ 新功能
- (无)

### 🐛 问题修复
- (无)

### 📦 其他变更
- [0;32m[INFO][0m 获取提交记录范围: v1.0.14..HEAD
- Fix all 11 identified issues

## v1.0.14 - 2026-08-19

### ✨ 新功能
- error messages now show as Alert dialog (stays until dismissed)

### 🐛 问题修复
- (无)

### 📦 其他变更
- [0;32m[INFO][0m 获取提交记录范围: v1.0.13..HEAD

## v1.0.13 - 2026-08-19

### ✨ 新功能
- (无)

### 🐛 问题修复
- RootHelper posix_spawn failure - sign with entitlements

### 📦 其他变更
- [0;32m[INFO][0m 获取提交记录范围: v1.0.12..HEAD

## v1.0.12 - 2026-08-19

### ✨ 新功能
- (无)

### 🐛 问题修复
- (无)

### 📦 其他变更
- [0;32m[INFO][0m 获取提交记录范围: v1.0.11..HEAD
- ci: eliminate all 7 warnings

## v1.0.11 - 2026-08-19

### ✨ 新功能
- bundle real tool binaries for full functionality

### 🐛 问题修复
- (无)

### 📦 其他变更
- [0;32m[INFO][0m 获取提交记录范围: v1.0.10..HEAD

## v1.0.10 - 2026-08-19

### ✨ 新功能
- (无)

### 🐛 问题修复
- align entitlements & bundle ID with reference app (DG管理器)
- compile error (missing emptyState) and ldid entitlements signing
- app list loading, account management, and TG link

### 📦 其他变更
- [0;32m[INFO][0m 获取提交记录范围: v1.0.7..HEAD
- chore(release): v1.0.9 [skip ci]
- chore(release): v1.0.8 [skip ci]

## v1.0.9 - 2026-08-19

### ✨ 新功能
- (无)

### 🐛 问题修复
- compile error (missing emptyState) and ldid entitlements signing
- app list loading, account management, and TG link

### 📦 其他变更
- [0;32m[INFO][0m 获取提交记录范围: v1.0.7..HEAD
- chore(release): v1.0.8 [skip ci]

## v1.0.8 - 2026-08-19

### ✨ 新功能
- (无)

### 🐛 问题修复
- app list loading, account management, and TG link

### 📦 其他变更
- [0;32m[INFO][0m 获取提交记录范围: v1.0.7..HEAD

## v1.0.7 - 2026-08-19

### ✨ 新功能
- (无)

### 🐛 问题修复
- replace system() with posix_spawnp to resolve .deb compilation failure
- resolve 303 Unable to locate main binary error

### 📦 其他变更
- [0;32m[INFO][0m 获取提交记录范围: v1.0.6..HEAD
- chore(release): v1.0.6 [skip ci]
- chore(release): v1.0.5 [skip ci]

## v1.0.6 - 2026-08-19

### ✨ 新功能
- (无)

### 🐛 问题修复
- resolve 303 Unable to locate main binary error

### 📦 其他变更
- [0;32m[INFO][0m 获取提交记录范围: v1.0.6..HEAD
- chore(release): v1.0.5 [skip ci]

## v1.0.5 - 2026-08-19

### ✨ 新功能
- (无)

### 🐛 问题修复
- C bridge for posix_spawn - Swift blocks process spawning on iOS

### 📦 其他变更
- [0;32m[INFO][0m 获取提交记录范围: v1.0.5..HEAD
- chore(release): v1.0.4 [skip ci]

## v1.0.4 - 2026-08-19

### ✨ 新功能
- (无)

### 🐛 问题修复
- (无)

### 📦 其他变更
- [0;32m[INFO][0m 获取提交记录范围: v1.0.5..HEAD

## v1.0.3 - 2026-08-19

### ✨ 新功能
- (无)

### 🐛 问题修复
- (无)

### 📦 其他变更
- [0;32m[INFO][0m 获取提交记录范围: v1.0.4..HEAD

## v1.0.2 - 2026-08-19

### ✨ 新功能
- (无)

### 🐛 问题修复
- CacheCleaner unused variable, deb fallback to placeholder

### 📦 其他变更
- [0;32m[INFO][0m 获取提交记录范围: v1.0.2..HEAD
- chore(release): v1.0.1 [skip ci]

## v1.0.1 - 2026-08-19

### ✨ 新功能
- (无)

### 🐛 问题修复
- (无)

### 📦 其他变更
- [0;32m[INFO][0m 获取提交记录范围: v1.0.2..HEAD

## v1.0.0 - 2026-08-19

### ✨ 新功能
- initial GhostKit v1.0.0

### 🐛 问题修复
- CI/CD detached HEAD, Makefile common.mk, xcodegen support

### 📦 其他变更
- [0;32m[INFO][0m 获取全部提交记录
- docs(changelog): 自动更新日志 [skip ci]
- Initial commit

## v1.0.0 - 2026-08-19

### ✨ 新功能
- initial GhostKit v1.0.0

### 🐛 问题修复
- (无)

### 📦 其他变更
- [0;32m[INFO][0m 获取全部提交记录
- Initial commit

## v1.0.0 - 2026-08-19

### ✨ 新功能

- feat: 初始 GhostKit 项目结构
- feat: Keychain 管理器（清理/深度清理/删除/恢复/备份）
- feat: 标识符管理器（IDFA 刷新/变更/显示）
- feat: 一键新机功能
- feat: 缓存清理器（系统残留/数据库/数据目录/Cookie/粘贴板）
- feat: 应用列表管理（列表/搜索/卸载）
- feat: App Store 账号管理
- feat: 一键允许粘贴权限
- feat: 游戏画质配置（PUBG/王者荣耀等）
- feat: RootHelper 提权工具
- feat: GitHub Actions CI/CD 自动编译
- feat: roothide/RELAXIN 越狱适配
- feat: TrollStore .tipa 支持
